...
public class SensitivityUtils {
	...
	/**
	 * Returns the call context sensitive forward or reverse data flow from the
	 * starting point in the given context.
	 * 
	 * @param context
	 * @param from
	 * @param forward
	...
	public static Graph sensitiveFlow(Graph context, Graph from, boolean forward) {
		...
		// A history of method call sites, built throughout the traversal. Its
		// keys
		...
		// Stack variables used inside the loops below, allocated here for
		// efficiency
		Graph lcFlow, ipFlow;
		...
		do {
			...
			lcFlow = forward ? lForward(dfLocalContext, nodes, false, Integer.MAX_VALUE) : lReverse(dfLocalContext,
					nodes, false, Integer.MAX_VALUE);
			nodes.addAll(lcFlow.nodes());
			edges.addAll(lcFlow.edges());

			// From the current nodes, traverse the DF_INTERPROCEDURAL nodes 1
			// step
			ipFlow = forward ? lForward(dfInterContext, nodes, false, 1) : lReverse(dfInterContext, nodes, false, 1);
			...
			for (Iterator<GraphElement> it = toExamine.iterator(); it.hasNext();) {
				...
				// Else if the destination is a parameter or return value
					...
					if ((destIsParam && forward) || (originIsReturn && !forward)) {
					...
					// Else if we're on the "trailing" side of the traversal,
					// and the object
					// instance passed is consistent with the object instance
					// history, and
					// the call context is consistent with the call context
					// history
	...
	/**
	 * Given a control flow block (corresponds to a call site) and a method
	 * node, adds the method node block to the CF block's history.
	 * 
	 * @param whitelist
	 * @param cfBlock
	 * @param other
	 */
	private static void addToHistory(HashMap<GraphElement, HashSet<GraphElement>> whitelist, GraphElement cfBlock,
			GraphElement other) {
	...
	/**
	 * Checks to see if the other element exists in the given CF block's
	 * whitelist.
	 * 
	 * @param history
	 * @param cfBlock
	 ...
	 * @return
	 */
	private static boolean isInHistory(HashMap<GraphElement, HashSet<GraphElement>> history, GraphElement cfBlock,
			GraphElement other) {
	...
	/**
	 * Given a GraphElement in a View, walks declares backwards until the
	 * container node of the given type is found. If none is found, returns null
	...
	private static GraphElement firstDeclarator(Graph declaresContext, GraphElement block, String type) {
		...
		while (true) {
			...
			// If no nodes are in the result, then we reached the end of the
			// road without finding
			// a method. (This should never happen)
			if (blockGraph.nodes().size() == 0)
				return null;

			// Should definitely only be one node in this set. Can't be declared
			// by multiple
			// things.
			GraphElement nextBlock = blockGraph.nodes().getFirst();

			if (nextBlock.tags().contains(type))
				return nextBlock;
		}
	}
}