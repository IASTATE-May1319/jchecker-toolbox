...
object TargetFlowChecker extends App {
  ...
  def highlightTargetFlow(envelope: Q, src: String, dest: String) = {
    ...
    var sourceNodes = edges(Edge.ANNOTATION).reverseStep(universe.selectNode("name", src) intersection allAnnotationNodes);
    var destinationNodes = universe.selectNode("name", dest) intersection allAnnotationNodes;

    var targetFlow = (universe.between(sourceNodes, destinationNodes) union sourceNodes);
    var annotNodes = allAnnotationNodes intersection targetFlow;

    var annotEdges = edges(Edge.ANNOTATION).reverseStep(allAnnotationNodes) intersection targetFlow;
